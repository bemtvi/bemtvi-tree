#!/usr/bin/env bash
#
# Verify doc/nxvim-tree.txt is up to date with the Markdown it is generated
# from. Regenerates it and fails if the result differs from what is committed — so a
# push can't ship a help file that no longer matches doc/nxvim-tree.md. Wired
...
OUTPUT="doc/nxvim-tree.txt" # must match OUTPUT in gen-vimdoc.sh
